...
description: Orquesta el flujo ASDD completo. Fase 1 (Spec) → Fase 2 (Backend) → Fase 3 (Tests) → Fase 4 (QA).
...
```
[FASE 1 — SECUENCIAL]
  spec-generator → .opencode/specs/<feature>.spec.md  (DRAFT → APPROVED)
...
## Proceso
1. Busca `.opencode/specs/<feature>.spec.md`
...
3. Cuando Fase 2 completa → lanza Fase 3 (Task tests-be)
4. Cuando Fase 3 completa → lanza Fase 4 (qa-agent)
5. Actualiza spec a `IMPLEMENTED` y reporta estado final

## Comando status
Al recibir `status`: lista specs en `.opencode/specs/` con su estado y próxima acción pendiente.

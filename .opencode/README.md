# ASDD Framework — Guía de Uso (Java Spring Boot)
...
```
Requerimiento → Spec → [Backend ∥ DB] → Tests → QA → Doc (opcional)
```
...
| Java 17+ | JDK para compilación |
| Gradle | Build tool del proyecto |

El archivo `.vscode/settings.json` ya configura el auto-descubrimiento de agentes, skills e instructions. Si no existe, créalo con las rutas correspondientes a `.opencode/`.

---

## Onboarding — nuevo proyecto

Al copiar `.opencode/` y `docs/` a un proyecto nuevo, completa estos archivos **en orden** antes de usar cualquier agente:

| # | Archivo | Qué escribir |
|---|---------|-------------|
| 1 | `README.md` (raíz del proyecto) | Stack, arquitectura, comandos (`build`, `test`, `run`), variables de entorno |
| 2 | `.opencode/instructions/backend.instructions.md` | Lenguaje, framework, base de datos, herramientas aprobadas |
| 3 | `.opencode/instructions/backend.instructions.md` | Capas, módulos, bounded contexts |
| 4 | `AGENTS.md` (Diccionario de Dominio) | Términos canónicos del negocio (glosario) |
| 5 | `copilot-instructions.md` (DoR + DoD ya incluidos) | Criterios DoR y DoD del equipo |

Una vez completados, los agentes tienen todo el contexto para operar de forma autónoma.

**No modificar**: `agents/`, `skills/`, `instructions/`, `.opencode/docs/lineamientos/`, `copilot-instructions.md`, `AGENTS.md`
...
El agente genera: casos Gherkin, matriz de riesgos, plan de automatización y (si hay SLAs) plan de performance.
...
| `@QA Agent` | Fase 4 | Gherkin, riesgos, automatización y análisis de performance |
...
| `/unit-testing` | Test Engineer Backend | Genera suite de tests (JUnit 5 + Mockito) |
...
| `/qa-task` | Ejecutar el flujo QA (Gherkin + riesgos + automatización + performance) |
...
|---|---|
| `**/*.java` | `instructions/backend.instructions.md` |
| `**/test/**/*.java` | `instructions/tests.instructions.md` |
...
| Documento | Contenido |
|---|---|
| `.opencode/docs/lineamientos/dev-guidelines.md` | Clean Code, SOLID, API REST, Seguridad, Observabilidad |
| `.opencode/docs/lineamientos/qa-guidelines.md` | Estrategia QA, Gherkin, Riesgos, Automatización, Performance |
| `.opencode/docs/lineamientos/guidelines.md` | Referencia rápida de estándares: código, tests, API, Git |
...
└── .opencode/                         ← framework Copilot (auto-contenido para compartir)
    ...
    │
    ├── skills/                      ← 8 skills (/comando en Copilot Chat)
    │   ├── asdd-orchestrate/
    │   ├── generate-spec/
    │   ├── implement-backend/
    │   ├── unit-testing/
    ...
    │   └── qa-guidelines.md
    │
    ├── prompts/                     ← 7 prompts (/nombre en Copilot Chat)
    │
    ├── instructions/                ← aplicadas automáticamente por contexto de archivo
    │   ├── backend.instructions.md  ← applyTo: **/*.java
    │   └── tests.instructions.md   ← applyTo: **/test/**/*.java
    │
